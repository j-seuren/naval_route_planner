...
from support import inputKC, inputWeather, inputECA, inputGulf, inputSalLim
...
def single_experiment(exp, startEnd, depDate, fileString, algorithm, generalFP, saveFig=True):
    ...
    for itr in range(ITERS):
        ...
        t0 = time.time()
        raw = PLANNER.compute(startEnd, startDate=depDate, recompute=True, weather=weather, current=current, algorithm=algorithm)
        t1 = time.time() - t0
...
def multiple_experiments(inputDict, exp, algorithm='NSGA2'):
    ...
    [os.makedirs(directory) for directory in testDirs if not os.path.exists(directory)]
