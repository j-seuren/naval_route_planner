...
def draw_basemap(individuals, vessel, m=None, uin=None, vin=None, lons=None, lats=None, color=None):
    col_map = cm.rainbow
    ...
    for ind in individuals:
        ...
        for i, e in enumerate(edges):
            if color:
                m.drawgreatcircle(e[0][0], e[0][1], e[1][0], e[1][1], linewidth=2, color=color, zorder=1)
            else:
                m.drawgreatcircle(e[0][0], e[0][1], e[1][0], e[1][1], linewidth=2, color=col_map(normalized_speeds[i]),
                                  zorder=1)
...
if __name__ == "__main__":
    ...
    for p in range(len(sp)):
        ...
        individuals = []
        for i in range(sp[p]):
            with open('output/paths/18_40_37_population_p{}_sp{}'.format(p, i), 'rb') as f:
        ...
        if p == 0:
            ...
            m = draw_basemap(individuals, vessel, uin=u_out, vin=v_out, lons=lons, lats=lats)
        else:
